...
\begin{itemize}
    ...
    \item 设计「拉取代码 → 沙箱运行 → 日志诊断 → 自动修复建议 → 复现报告」核心任务流，明确用户输入、系统状态、异常处理与最终交付物
    \item 主导前端 UI 与交互设计，将复杂的复现流程拆解为任务创建、沙箱运行、日志反馈、修复建议与报告输出等清晰页面状态，提升用户对 Agent 执行过程的可理解性
    \item 与工程成员对齐 Docker 沙箱、Agent 工作流、实时任务状态、日志分析与报告输出等能力边界，推进可演示原型落地
\end{itemize}

\vspace{0.18em}
\projectentry{早期产品与技术原型探索}{2025 -- 2026}{产品探索 / 开发实践}
\begin{itemize}
    \item \textbf{AIHub}：全栈独立开发多模型 AI 工作台移动端 MVP，基于 Expo / React Native 验证 ChatGPT、Claude、DeepSeek 等模型统一入口、提示词模板与 AI 使用教学流程
    \item \textbf{Arcflow}：全栈独立开发 AI 工作流工具，将写作、调研、开发等复杂任务拆成步骤化流程，探索「从聊天到流程化产出」的 AI 产品交互形态
    \item \textbf{EventLite}：大二团队项目，主导校园活动管理平台前端 UI 与交互设计，参与 Java Spring Boot 后端接口、数据模型、用户/活动管理等模块开发
\end{itemize}
